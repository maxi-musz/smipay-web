...
import type { SupportFilters as SupportFiltersType } from "@/types/admin/support";

interface SupportFiltersProps {
  filters: SupportFiltersType;
  onSearch: (value: string) => void;
  onFilterChange: (updates: Partial<SupportFiltersType>) => void;
  onReset: () => void;
  total: number;
}
    ...
    <div className="bg-dashboard-surface rounded-xl border border-dashboard-border/60">
      ...
      <div className="flex items-center gap-2 px-3 py-2.5">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Search ticket #, subject, email, phone..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => onFilterChange({ support_type: e.target.value })}
                    className="w-full px-2.5 py-1.5 text-xs bg-dashboard-bg border border-dashboard-border/60 rounded-lg text-dashboard-heading focus:outline-none focus:ring-2 focus:ring-brand-bg-primary/20 appearance-none"
                  >
