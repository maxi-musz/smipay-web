...
export function UsersListSort({ listSort, onChange, className = "" }: Props) {
  return (
    <div
      className={`flex flex-wrap items-center gap-2 text-xs shrink-0 sm:ml-2 ${className}`.trim()}
    >
      ...
      <select
        ...
        className="min-w-0 w-full max-w-full sm:w-auto sm:max-w-[min(280px,36vw)] md:max-w-[280px] px-2.5 py-1.5 text-xs bg-dashboard-surface border border-dashboard-border/60 rounded-lg text-dashboard-heading focus:outline-none focus:ring-2 focus:ring-brand-bg-primary/20"
      >
