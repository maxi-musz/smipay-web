...
import { MARKUP_SERVICE_TYPES } from "@/types/admin/markup";
...
export function MarkupSettings({
  ...
  const [configForm, setConfigForm] = useState({
    ...
    default_percentage_friendlies: config.default_percentage_friendlies ?? "",
    min_amount_to_apply_markup: config.min_amount_to_apply_markup ?? 0,
  });
  ...
  const [ruleEdits, setRuleEdits] = useState<
    Record<string, Partial<Pick<MarkupRule, "percentage" | "percentage_friendlies" | "min_amount_to_apply_markup">>>
  >({});
  ...
  const [ruleToggleSuccess, setRuleToggleSuccess] = useState<string | null>(null);
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      const payload: MarkupConfigPayload = {};
      ...
      if (configForm.default_percentage !== config.default_percentage)
        payload.default_percentage = Number(configForm.default_percentage);
      const friendliesVal =
        configForm.default_percentage_friendlies === ""
          ? null
          : Number(configForm.default_percentage_friendlies);
      if (friendliesVal !== (config.default_percentage_friendlies ?? null))
        payload.default_percentage_friendlies = friendliesVal;
      const minVal =
        configForm.min_amount_to_apply_markup === 0
          ? null
          : configForm.min_amount_to_apply_markup;
      if (minVal !== (config.min_amount_to_apply_markup ?? null))
        payload.min_amount_to_apply_markup = minVal;

      if (Object.keys(payload).length === 0) return;
      await adminMarkupApi.updateConfig(payload);
  ...
  const handleToggleProgram = async () => {
    ...
    try {
      await adminMarkupApi.updateConfig({
        is_active: !config.is_active,
      });
      onSaved();
      setToggleSuccess(true);
      setTimeout(() => setToggleSuccess(false), 2500);
    } catch (err) {
      setConfigError(
  ...
  const handleSeedRules = async () => {
    setSeeding(true);
    try {
      await adminMarkupApi.seedRules();
      onSaved();
    } catch {
      // non-critical
    } finally {
  ...
  const handleToggleRule = async (rule: MarkupRule) => {
    setRuleToggling(rule.id);
    setRuleToggleSuccess(null);
    try {
      await adminMarkupApi.updateRule(rule.id, {
        is_active: !rule.is_active,
      });
      onSaved();
      setRuleToggleSuccess(rule.id);
      setTimeout(() => setRuleToggleSuccess(null), 2500);
    } catch {
      // error could surface via onSaved refetch / parent
    } finally {
  ...
  const getRuleEdit = useCallback(
    (rule: MarkupRule) => {
      ...
      return {
        ...
        percentage_friendlies:
          edit?.percentage_friendlies !== undefined
            ? edit.percentage_friendlies
            : rule.percentage_friendlies,
        min_amount_to_apply_markup:
          edit?.min_amount_to_apply_markup !== undefined
            ? edit.min_amount_to_apply_markup
            : rule.min_amount_to_apply_markup,
      };
    },
    [ruleEdits],
  );
  ...
  const setRuleField = (
    ruleId: string,
    field: "percentage" | "percentage_friendlies" | "min_amount_to_apply_markup",
    value: number | null,
  ) => {
    setRuleEdits((prev) => ({
      ...prev,
  ...
  const handleSaveRule = async (rule: MarkupRule) => {
    const edit = ruleEdits[rule.id];
    if (!edit) return;
    ...
    try {
      ...
      if (edit.percentage_friendlies !== undefined)
        payload.percentage_friendlies = edit.percentage_friendlies;
      if (edit.min_amount_to_apply_markup !== undefined)
        payload.min_amount_to_apply_markup = edit.min_amount_to_apply_markup;

      if (Object.keys(payload).length === 0) return;
      await adminMarkupApi.updateRule(rule.id, payload);
      setRuleEdits((prev) => {
        const next = { ...prev };
        delete next[rule.id];
        return next;
  ...
  const handleDeleteRule = async () => {
    ...
    try {
      await adminMarkupApi.deleteRule(deleteTarget.id);
  ...
  const configDirty =
    ...
    (configForm.default_percentage_friendlies === ""
      ? null
      : Number(configForm.default_percentage_friendlies)) !==
      (config.default_percentage_friendlies ?? null) ||
    (configForm.min_amount_to_apply_markup === 0
      ? null
      : configForm.min_amount_to_apply_markup) !==
      (config.min_amount_to_apply_markup ?? null);

  const serviceLabel = (type: MarkupServiceType) =>
    MARKUP_SERVICE_TYPES.find((s) => s.value === type)?.label ?? type;

  const showSeedButton = rules.length < 7;
    ...
    <div className="space-y-4">
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div
              className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 ${config.is_active ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-500"}`}
            >
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-dashboard-muted max-w-md">
                {config.is_active
                  ? "Users pay provider price + markup. We earn the markup."
                  : "Users pay provider price only. No markup revenue."}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {configSaving && (
              <span className="text-xs text-dashboard-muted flex items-center gap-1">
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                Saving…
              </span>
            )}
            <AnimatePresence mode="wait">
              {toggleSuccess && !configSaving && (
                <motion.span
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors disabled:opacity-60 disabled:cursor-not-allowed ${config.is_active ? "bg-emerald-500" : "bg-slate-300"}`}
            >
              <span
      ...
      >
        ...
        <p className="text-[11px] text-dashboard-muted mb-3">
          Apply unless a service rule overrides. Default % (Friendlies) is
          optional — leave empty to use same as default %.
        </p>
        {configError && (
        ...
        <div className="grid grid-cols-2 gap-3">
          <InputField
            ...
            onChange={(v) =>
              setConfigForm((p) => ({
                ...p,
                default_percentage: typeof v === "number" ? v : Number(v) || 0,
              }))
            }
            suffix="%"
            step={0.5}
          ...
          <InputField
            ...
            onChange={(v) =>
              setConfigForm((p) => ({
                ...p,
                default_percentage_friendlies: v === "" ? "" : String(v),
              }))
            }
            suffix="%"
            step={0.5}
          ...
          <div className="col-span-2">
            <InputField
              label="Min Amount to Apply Markup (NGN)"
              value={configForm.min_amount_to_apply_markup}
              onChange={(v) =>
                setConfigForm((p) => ({
                  ...p,
                  min_amount_to_apply_markup:
            ...
            <p className="text-[10px] text-dashboard-muted mt-1">
              0 = no minimum; transactions below this get no markup.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-dashboard-border/60">
          ...
          {showSeedButton ? (
            ...
            <p className="text-[10px] text-dashboard-muted">
              Empty % (Friendly) = same as regular %. Empty Min = global.
            </p>
          )}
        </div>
        ...
        {rules.length === 0 ? (
          <div className="px-4 py-8 text-center">
            ...
            <p className="text-xs text-dashboard-muted max-w-xs mx-auto">
              Click &ldquo;Seed All Rules&rdquo; to create a row for every
              service (airtime, data, cable, etc.). All start at 0% and OFF.
              Then set percentages and turn each on.
            </p>
          </div>
        ) : (
          <div className="divide-y divide-dashboard-border/40">
            <div className="hidden lg:grid grid-cols-[1fr_80px_70px_90px_90px_80px] gap-3 px-4 py-2 text-[10px] font-semibold text-dashboard-muted uppercase tracking-wider bg-dashboard-bg/50">
              <span>Service</span>
              <span>Active</span>
              <span>%</span>
              <span>% Friendly</span>
            ...
            {rules.map((rule) => {
              ...
              const toggling = ruleToggling === rule.id;
              const toggleJustSaved = ruleToggleSuccess === rule.id;
              ...
              return (
                ...
                >
                  <div className="lg:hidden space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {toggling && (
                          <span className="text-[10px] text-dashboard-muted flex items-center gap-1">
                            <Loader2 className="h-3 w-3 animate-spin" />
                            Saving…
                          </span>
                        )}
                        {toggleJustSaved && !toggling && (
                        ...
                        <button
                          ...
                          disabled={toggling}
                          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors disabled:opacity-60 disabled:cursor-not-allowed ${rule.is_active ? "bg-emerald-500" : "bg-slate-300"}`}
                        >
                          <span
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                      ...
                      <MiniInput
                        label="% Friendly"
                        value={edited.percentage_friendlies}
                        onChange={(v) =>
                          setRuleField(rule.id, "percentage_friendlies", v)
                        }
                      />
                      <MiniInput
                        label="Min ₦"
                        value={edited.min_amount_to_apply_markup}
                        onChange={(v) =>
                          setRuleField(
                            rule.id,
                            "min_amount_to_apply_markup",
                  ...
                  <div className="hidden lg:grid grid-cols-[1fr_80px_70px_90px_90px_80px] gap-3 items-center">
                    <span className="text-xs font-semibold text-dashboard-heading">
                      {serviceLabel(rule.service_type)}
                    </span>
                    <div className="flex items-center gap-1.5">
                      {toggling && (
                        <Loader2 className="h-3 w-3 animate-spin text-dashboard-muted shrink-0" />
                      )}
                      {toggleJustSaved && !toggling && (
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-dashboard-heading">
                    {config.is_active ? "Disable" : "Enable"} Markup
                  </h3>
                  <p className="text-xs text-dashboard-muted">
                    {config.is_active
                      ? "Users will pay provider price only. No markup revenue."
                      : "Users will pay provider price + markup. Affects all services immediately."}
                  </p>
                </div>
              </div>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-xs text-dashboard-muted mb-4">
                ...
                This service will use the global default % (
                {config.default_percentage}%) and global min amount.
              </p>
              <div className="flex justify-end gap-2">
                <button
                  type="button"
...
function InputField({
  ...
  const numValue = isNumber ? value : (value === "" ? "" : Number(value));
  return (
    <div>
      <label className="block text-[10px] font-medium text-dashboard-muted mb-1">
        {label}
        {optional && (
          <span className="text-dashboard-muted/80 ml-1">(optional)</span>
        )}
      </label>
