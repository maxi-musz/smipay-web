...
export function PurchaseConfirmationModal({
  ...
  const cleanedNetworkName =
    networkName.replace(" Data", "").replace(" (SME)", "") || "N/A";
  const logo = serviceID ? getNetworkLogo(serviceID) || networkImage : networkImage;
          ...
          <div className="flex items-center justify-between py-3 border-b border-slate-100">
            ...
            <div className="flex items-center gap-2">
              {logo ? (
                ...
                <img
                  src={logo}
                  alt={cleanedNetworkName}
                  className="h-5 w-5 rounded-full object-contain"
                />
              ) : (
                <Wifi className="h-4 w-4 text-slate-400" />
              )}
              <span className="text-sm font-semibold text-slate-900">Data</span>
            </div>
          </div>

          <div className="flex items-center justify-between py-3 border-b border-slate-100">
            <span className="text-sm text-slate-500">Data Plan</span>
            <span className="text-sm font-semibold text-slate-900 text-right max-w-[65%] truncate">
              {planName}
            </span>
          </div>
            ...
            <div className="flex items-center justify-between py-3 border-b border-slate-100">
              <span className="text-sm text-slate-500">
                ...
                <span className="text-slate-400">
                  (₦
                  {cbBalanceNum.toLocaleString("en-NG", {
              ...
              <div className="flex items-center gap-2.5">
                <span className="text-sm font-semibold text-slate-900 tabular-nums">
                  -₦
                  {cbDeduction.toLocaleString("en-NG", {
                    minimumFractionDigits: 2,
                  })}
                </span>
                <button
